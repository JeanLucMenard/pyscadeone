...
from .expressions import Pattern, Literal
...
class LHSItem(common.SwanItem):
    ...
    @property
    def identifier(self) -> Union[common.Identifier, None]:
...
class ExprEquation(common.Equation):
    ...
    *equation* ::= *lhs* = *expr*"""
    ...
    def __init__(self, lhs: EquationLHS, expr: common.Expression) -> None:
        super().__init__()
        self._lhs = lhs
        self._expr = expr
...
class DefByCase(common.Equation, ABC):
    ...
    def __init__(self, lhs: Optional[EquationLHS] = None, name: Optional[str] = None) -> None:
    ...
    def name(self) -> Union[str, None]:
        """Returns **activate** name or None if no name."""
        return self._name

    def get_luid(self) -> str:
        """Returns proper LUID string."""
        if not self.name:
            return ""
        luid = common.Luid(self.name)
        luid = " " + common.Markup.to_str(
            str(luid),
            is_protected=not common.Luid.is_valid(luid.value),
            markup=common.Markup.NoMarkup,
        )
        return luid
...
class Identification(common.SwanItem):
    """State identification:

    *identification* ::= ID | LUID [[ ID ]]
    ...
    def __init__(
        self, luid: Optional[common.Luid] = None, id: Optional[common.Identifier] = None
    ) -> None:
        super().__init__()
        self._luid = luid
        self._id = id
    ...
    def is_undef(self) -> bool:
        """True when neither LUID nor ID is defined."""
...
class Target(common.SwanItem):
    ...
    def __init__(self, target: Identification, is_resume: Optional[bool] = False) -> None:
        super().__init__()
...
class Arrow(common.SwanItem):
    ...
    @property
    def action(self) -> Union["Scope", None]:
...
class ForkPriorityList(Fork):
    ...
    *fork* ::=  {{ *fork_priority* }} **end**
    ...
    def __init__(self, fork_with_prio_list: List[ForkWithPriority]) -> None:
        super().__init__()
...
class Transition(common.SwanItem):
...
class TransitionDecl(StateMachineItem):
    ...
    def __str__(self) -> str:
        ...
        priority = str(self.priority) if self.priority else " "
        return f":{priority}:{id} {kind} {self.transition}"
