...
from ansys.scadeone.common.assets import SwanString, SwanCode
...
def cmp_string(orig, new, no_markup=False, diff=False):
    ...
    if not cmp and diff:
        ...
        res = "".join(d.compare(orig_strip.splitlines(keepends=True),
...
def check_fragment(rule, fragment: Union[SwanCode, str], no_markup=False, diff=False):
    ...
    rule :parser function
        rule to be applied
    fragment : Union[SwanCode, str]
        Swan code
    no_markup : bool, optional
        suppress markups in fragma, by default False
    diff : bool, optional
...
def check_body(code: str, no_markup=False, diff=False):
    ...
    (obj, info) = parser.module_body(swan)
    obj_str = str(obj)
    cmp_string(swan.content(), obj_str, no_markup, diff)
...
class TestDecl:
    def test_sensor(self):
        ...
        assert sensor.identifier.value == 'S'
        assert isinstance(sensor.type, S.PredefinedTypeExpr)
        assert sensor.type.predefined == S.PredefinedTypes.Int32
    ...
    def test_constant_no_init(self):
        ...
        assert constant.identifier.value == 'C'
        assert isinstance(constant.type, S.PredefinedTypeExpr)
        assert constant.type.predefined == S.PredefinedTypes.Int32
    ...
    def test_constant_init(self):
        ...
        left = value.left
        assert isinstance(left, S.LiteralExpr)
        assert left.is_integer
        assert left.value == "41"

    def decl_gen_test(self, decl: str):
        """Test a global declaration of kind kind
        (sensor, const, type) in decl"""
        swan = SwanString(decl)
        py_decl = parser.declaration(swan)
        def check(py_decl):
            if isinstance(py_decl, S.TypeDeclarations):
                return True
            if isinstance(py_decl, S.SensorDeclarations):
                return True
            if isinstance(py_decl, S.ConstDeclarations):
                return True
            if isinstance(py_decl, S.GroupDeclarations):
                return True
            if isinstance(py_decl, S.UseDirective):
                return True
            assert False
        assert check(py_decl)
        as_str = str(py_decl)
        new_decl = parser.declaration(SwanString(as_str))
        assert check(new_decl)
        new_str = str(new_decl)
    ...
    def test_types(self):
        ...
        check_decl('type T = signed <<42>>;')
        check_decl('type T = unsigned <<2 * N + 1>>;')
        check_decl(" type T = {a: bool};")
        check_decl("type T = {a: bool, b: {c: bool, d: int8, e: char}};")
        self.decl_gen_test("""type T = {
                                        a: bool,
                                        b: {
                                             c: bool,
                                             d: int8,
        ...
        check_decl('type T = enum {A, B, C};')
        check_decl('type T = A {};')
        check_decl('type T = A { X };')
        check_decl("""type either3 =
                                Either1 {}
                                | Either2 { int32 }
...
class TestExpression:
    def gen_expr_test(self, expr: str):
        ...
        py_swan = SwanString(py_expr_str)
        py_expr2 = parser.expression(py_swan)
        py_expr2_str = str(py_expr2)
        cmp_string(expr, py_expr2_str)
    ...
    @pytest.mark.parametrize("group_expr",
                             [
                                "(42, a: 666)",
                                "42 .(a)",
                                "42 .(a: b)",
...
node FOO () returns ()
{
    var clock probe X1: int8 default = 42 last = 666;
        X2;
    var X3;
}
"""
...
class TestEquation:
    ...
    def test_simple_eq(self, make_lhs, make_path_identifier):
        eq = S.ExprEquation(make_lhs(1),
                            S.PathIdExpr(make_path_identifier()))
...
() : activate #activate0
    ...
    @pytest.mark.parametrize(
            ...
            [
                ...
                let x = 42;""",
                # transition Swan
                ':1: unless if (true) resume S1;',
                ':2: unless if (true) restart S1;',
                ':3: unless if (true) { let x = 1; } restart S1;',
                ':4: unless if (true) :1: else restart S1 end;',
                ':5: unless if (true) :1: if (false) restart S1 end;',
                """:6: unless if (true)
                       :1: if (A) restart SA
                       :1: if (B) restart SB
                       :2: else
...
sensor S1: int8; S2: int8;""",
    ...
    def test_module_name(self, name, expected):
        code = SwanString("type T;", name)
        (obj, info) = parser.module_body(code)
        obj_name = str(obj.name)
        assert obj_name == expected

    def test_signature(self,
                       make_identifier,
                       make_path_identifier,
                       make_var_decl,
                       make_type_var):
...
r"""node ID1 (
  ID2: bool
        ...
        sig_str = str(sig)
        oracle = \
r"""node ID1 <<ID2, ID3>> () returns () specialize ID4::ID5::ID6;"""
        assert sig_str == oracle
        ...
        sig = S.Signature(
            ...
            [],
            [],
            constraints=[S.TypeConstraint([make_type_var(), make_type_var()], S.NumericKind.Signed),
                         S.TypeConstraint([make_type_var()], S.NumericKind.Integer)]
        )
        sig_str = str(sig)
        oracle = \
r"""function ID1 () returns () where 'T1, 'T2 signed where 'T3 integer;"""
        assert sig_str == oracle
        ...
        sig_str = str(sig)
        oracle = \
"""function #pragma x #end #pragma y #end ID1 () returns ();"""
